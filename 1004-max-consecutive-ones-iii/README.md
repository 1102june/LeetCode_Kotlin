1.초기접근: 불변Sliding Window 풀듯이 그냥 먼저 k개의 0을 만나면 Stop하고 거기서부터 index를 증가시켜서 0만나면 더하고 1이면 빼고 느낌으로 가려했으나 이렇게 구성하는 순간부터 최대길이를 찾을수가 없는 구조 발생.
2. 어떻게 접근해야할지 모르겠어서 a가 0만나면 Zero++하고 그러다가 zero의 개수가 K보다 많아지는 순간 b가 출발하면서 0을 빼버림. 이제 최종적으로 b-a+1 중에서 최대길이 구하면 됨
